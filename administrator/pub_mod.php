		...
		if(isset($_FILES['pub']) && $_FILES['pub']['error'] == 0)
		{
			if ($_FILES['pub']['size'] <= 1073741824*3) // Verifier la taille du fichier 3GO MAX
			{
				...
				if (in_array($extension_telechargees, $extensions_autorisees))
				{
					...
					if(($p == 2 && empty($_POST['text']) && $width > 183) || ($p == 3 && empty($_POST['text']) && $width > 200) || (!empty($_POST['text']) == 2 && $width > 30))
					{
						if($p == 2 && empty($_POST['text']))
						{
							$red_height = $width / 183;
							$height = $height / $red_height;
							$width = 183;
						}
						elseif($p == 3 && empty($_POST['text']))
						{
...
htm_admin_header();
htm_menu();
htm_pub_mod($id, $p, $id_reg, $id_dep, $id_cat, $error);
htm_admin_footer();