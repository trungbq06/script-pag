...
if(!empty($_POST['opt_type1']) && empty($error))
{
	...
	if($prix_pack > 0)
	{
		...
		$_SESSION['item'] = $language['item9_page_paiement'];
		
		$prix_tva = (!empty($param_gen['tva_taux'])) ? $prix_pack * $param_gen['tva_taux'] / 100 : 0;
		
		$conn = db_connect();
		$id_achat = register_achat(3, 0, $id, 0, $jour, $annonce, 0, time(), $prix_pack, $prix_tva);
		$conn = null;
	...
	{
		$conn = db_connect();
		$id_achat = register_achat(3, 0, $id, 0, $jour, $annonce, 0, time(), '', '');
...
$title = $language['pay_pack_acc_title'];
$description = '';
$words = '';
$info_page = $language['pay_pack_acc_title'];
